...
ENGINE_VAR reload_mode;	// not used
...
ENGINE_EVENT on_frame;  // called every frame
ENGINE_EVENT on_message; // the engine window's message loop
ENGINE_EVENT on_reserved[16];

// engine pointers ///////////////////////////////////////////////////
ENGINE_VIEW		camera;			// predefined view
ENGINE_VIEW		light_view;	    // view used for clipping dynamic lights
...
ENGINE_ENTITY	level_ent;      // pseudo entity representing the level
ENGINE_PANEL    mouse_panel;    // panel under mouse
ENGINE_ long	epunused1[1];

ENGINE_ void*	render_d3dmaterial;	// current LPD3DMATERIAL9
ENGINE_ void*	render_d3dxeffect;	// current LPD3DXEFFECT
ENGINE_MATERIAL	render_material;	// current material 
ENGINE_BMAP		render_target;		// render target of the previous view stage
ENGINE_BMAP		render_stencil;		// stencil map of the previous view stage
ENGINE_ void*	render_zbuffer;     // render target depth buffer LPDIRECT3DSURFACE9
ENGINE_ long	epunused2[3];
ENGINE_CONTACT  hit;
...
ENGINE_ long  eunused2[2];
		long  (*CallFunc)(long,long,long,long,long);	// DLLLIB internal use only
#else // use normal pointers for lite-C
ENGINE_EVENT on_sendpacket;
ENGINE_EVENT on_receivepacket;
ENGINE_EVENT on_scanmessage;
ENGINE_ long  eunused2[5];
#endif
...
ENGINE_ char** pPaths;      // NULL-terminated array of path names
ENGINE_ void* pVars;	    // begin of the variables
ENGINE_ void* dunused2[4];	// not used
