...
#ifndef default_c
#define default_c
#include <acknex.h>  // standard C-Script objects
...
// initialize speed and distance
	...
	while (def_camera) 
	{
        ...
        if (key_alt==0) {
            aforce.pan = -5*(key_force.x + mouse_right*mouse_force.x);
            aforce.roll = 0;
        } else {
            aforce.pan = 0;
            aforce.roll = 5*(key_force.x + mouse_right*mouse_force.x);
        }
...
// activate default movement and settings in entity viewer mode
    ...
    else if (3 == run_mode) // entity
    {
        ...
        if (NULL != player) 
        {
            ...
            if (str_stri(sPlayerModel,".hmp")) {
                camera.z = 0.5 * player.max_z;
                def_move(); // terrain? -> move the camera only
            } else 
            {
...
// rotate and move the entity
                    if (!def_camera) 
                    {
                        player.pan += 5*time_step*(key_force.x + key_a-key_d + 5*mouse_right*mouse_force.x);
